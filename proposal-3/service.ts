...
>(
  ...
  if (!IS_PRODUCTION) {
    Object.keys(fsmConfig.states).forEach((state) => {
      const stateConfig = fsmConfig.states[state as TState['value']];
      if ('states' in stateConfig) {
  ...
  const initialStateConfig = fsmConfig.states[fsmConfig.initial];
  const [initialActions, processedContext] = handleActions(
    toArray(fsmConfig.states[fsmConfig.initial].entry).map((action) => {
      ...
      return { ...actionObject, meta: initialStateConfig?.meta }; // Initial state meta for entry
    }),
    initialContext,
